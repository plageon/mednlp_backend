...
from ..metrics import sts_metric, qic_metric, qqr_metric, qtr_metric, \
    ctc_metric, ee_metric, er_metric, re_metric, cdn_cls_metric, cdn_num_metric
from ..metrics import sts_commit_prediction, qic_commit_prediction, qtr_commit_prediction, \
    qqr_commit_prediction, ctc_commit_prediction, ee_commit_prediction, cdn_commit_prediction
from ..models import convert_examples_to_features, save_zen_model
            ...
            for step, item in enumerate(train_dataloader):
                ...
                if args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if score > best_score:
                        ...
                        cnt_patience += 1
                        self.logger.info("Early stopper counter: %s out of %s", cnt_patience, args.earlystop_patience)
...
class EETrainer(Trainer):
    ...
    def evaluate(self, model, do_test=False, test_dataset=None):
        args = self.args
        logger = self.logger
        if do_test:
    ...
    def predict(self, model, test_dataset, save_result=True):
        args = self.args
        logger = self.logger
        ...
        if save_result:
            ee_commit_prediction(dataset=test_dataset, preds=predicts, output_dir=args.data_dir)
        return predicts
        ...
        with open(os.path.join(args.output_dir, 'CMeIE_test.json'), 'w', encoding='utf-8') as f:
            for step, item in enumerate(test_dataloader):
                ...
                with torch.no_grad():
                    ...
                    sub_arg_list = self.data_processor.extract_arg(sub_start_logits.view(-1), sub_end_logits.view(-1),
                                                                   text_start_id, text_end_id,
                                                                   text, text_mapping)
                    obj_arg_list = self.data_processor.extract_arg(obj_start_logits.view(-1), obj_end_logits.view(-1),
                                                                   text_start_id, text_end_id,
                                                                   text, text_mapping)
    ...
    def evaluate(self, model):
        ...
        for step, item in enumerate(eval_dataloader):
            ...
            input_ids, token_type_ids, attention_mask, flag, label = input_ids.to(self.args.device), \
                                                                     token_type_ids.to(self.args.device), \
                                                                     attention_mask.to(self.args.device), \
                                                                     flag.to(self.args.device), label.to(
                self.args.device)
                    ...
                    for item in temp_re_dataset:
                        ...
                        if args.model_type == 'zen':
                            ...
                            logit = model(input_ids=input_ids.view(1, -1), token_type_ids=token_type_ids.view(1, -1),
                                          ...
                                          input_ngram_ids=input_ngram_ids.view(1, -1),
                                          ngram_token_type_ids=ngram_token_type_ids.view(1, -1),
                                          ngram_attention_mask=ngram_attention_mask.view(1, -1),
                                          ngram_position_matrix=ngram_position_matrix.view(1, ngram_max_length,
                                                                                           ngram_max_length))
                        else:
                            logit = model(input_ids=input_ids.view(1, -1), token_type_ids=token_type_ids.view(1, -1),
